...
router.get('/requestFacultyData',function(req,res){
  ...
  if(!cookies){
    errRequest("http://13.126.16.198:80/error/nodejsErr/faculty","cookies",err)
    res.status(401)
    res.end()
  } else {
    jwt.verify(cookies.user,'uit attendance login',function(err,decoded){
      if(err){
        errRequest("http://13.126.16.198:80/error/nodejsErr/faculty","jwt",err)
        res.status(401)
        res.end()
      } else {
        ...
        mongo.connect('mongodb://localhost:27018/data',function(err,db){
          if(err){
            errRequest("http://13.126.16.198:80/error/mongoErr/faculty","mongodb",err)
            db.close()
            res.status(500)
            res.end()
          } else {
          ...
          if(err){
            ...
            res.end()
              ...
              request({
                ...
                if(err){
                  errRequest("http://13.126.16.198:80/error/nodejsErr/faculty","request",err)
  ...
  if(!cookies){
    ...
    jwt.verify(cookies.user,'uit attendance login',function(err,decoded){
      if(err){
        ...
        mongo.connect('mongodb://localhost:27018/data',function(err,db){
          if(err){
            errRequest("http://13.126.16.198:80/error/nodejsErr/faculty","mongodb",err)
            db.close()
            res.status(500)
            res.end()
          } else {
