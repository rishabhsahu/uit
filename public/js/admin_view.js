var view = {
    ...
    if(model.info.faculties){
      model.info.faculties.forEach(function(x,i){
        if( (i+1-1)%3 == 0 || (i+1-1)%3 == 3 ){
          names += "<div class='col-xs-3 faculty_name' id='" + x.id + "' onclick='controller.facultySelected(event)' style='margin-top:15px;border:solid 1px rgba(160,160,160,.8);box-shadow:2px 2px 5px rgba(160,160,160,.4);border-radius:3px'>" + x.name + "</div>";
        } else {
          names += "<div class='col-xs-3 col-xs-offset-1 faculty_name' id='" + x.id + "' onclick='controller.facultySelected(event)' style='margin-top:15px;border:solid 1px rgba(160,160,160,.8);box-shadow:2px 2px 5px rgba(160,160,160,.4);border-radius:3px'>" + x.name + "</div>";
        }
      })
    }
  ...
  showAddBatchModal: function(){
    ...
    "<div class='row' style='margin-top:24px;'><div class='col-xs-12'><b>batch</b>: <input id='year' type='text' maxlength='4' size='4'><br>(batch should be ending year for the batch. ex: if batch ending in year 2014, then '2014')</div><div class='col-xs-12' style='margin-top:10px;'><b>class</b>: " + stri + "</div><div class='col-xs-12' style='margin-top:10px;'><b>Section</b>: <input type='text' maxlength='4' size='4'></div><div class='col-xs-4 col-xs-offset-5' style='margin-top:10px;'><input type='file' style='margin-top:10px;' accept='text/plain' name='student_list' id='student_list'></div><br></div></div><div class='col-xs-1 close' style='font-size:40px;font-weight:bold;cursor:pointer;padding:0px;' onclick='view.closeAddBatchModal()'>&times;</div></div><div class='row'><div type=button style='margin-top:25px;' class='col-xs-2 col-xs-offset-5'><button class=' btn btn-danger' onclick='controller.addNewBatch()'>Send</button></div></div></div>";
  ...
  showSmsModal: function(){
    document.getElementById("switch").innerHTML += "<div id='smsFacultyModal' class='text-center modal col-sm-12'><div class='row'><div class='col-sm-6 col-sm-offset-3 modalContent'><div class='row modelHeader'><div class='col-xs-1 col-xs-offset-11' style='font-size:24px;font-weight:bold;cursor:pointer;padding:0px;' onclick='view.closeSendFacultySmsModal()'>&times;</div><div class='col-sm-12'><h2>Write SMS</h2></div></div><hr><div class='row'><div class='col-xs-12' style='margin-top:10px;'><textarea id='sms-text' maxlength=160 rows=6 id='class' type='text'></textarea></div><br><button type=button style='margin-top:15px;' class='col-xs-2 col-xs-offset-5 btn btn-danger' onclick='controller.sendFacultySms()'>Send</button></div></div></div>";
  ...
  messageWholeClassModal: function(){
    ...
    var names = "";
    model.selectedBatch.students.forEach(function(x,i){
      names += "<div class='col-xs-8 col-xs-offset-2' style='color:rgb(90,90,90);padding-top:10px;padding-bottom:10px;margin-top:3px;margin-bottom:3px;background-color:lightgrey;border-bottom:solid 1px rgba(160,160,160,.5);cursor:pointer;border-radius:5px' onclick='view.selectName(event,1)' id='" + x.enroll_number + "'>" + x.name + "</div>";
    })
