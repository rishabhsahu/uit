var view = {
  loadHome: function(){
    var content = "<div class='col-xs-12' style='padding-top:5px;padding-bottom:5px;margin-top:5px;background-color:white;border: solid 1px rgba(200,200,200,.5);font-size:16px;text-transform:capitalize;color:rgba(0,0,0,.7)'>" + model.personalInfo.name + "</div><div class='col-xs-12'><div class='row' id='home_section'></div></div>";
    document.getElementById('main').innerHTML = content;
    var classes_today = "";
    model.classes_today.forEach(function(c,n){
      classes_today += "<div class='col-xs-3' style='color:rgba(0,0,0,.6);padding:0px;border-right:solid 1px rgba(10,10,10,.15)'><div class='row' style='padding:0px;margin:0px'><div class='col-xs-12' style='padding:0px;color:rgba(0,0,0,.7)'>" + c.class + "th<sub>class</sub></div><div class='col-xs-12' style='padding:0px;color:rgba(0,0,0,.5)'>" + c.period + "th<sub>per</sub></div></div></div>";
    })
    console.log(model);
    var content2 = "<div class='col-xs-12' style='padding-top:7px;background-color:white;border: solid 1px rgba(200,200,200,.5);'><div class='row'><div class='col-xs-12' style='font-size:16px'>3 Classes today</div></div><div class='row' style='margin-top:10px;margin-bottom:5px'>" + classes_today + "</div></div>";
  ...
  showBatchList: function(){
    ...
    model.personalInfo.current_classes.forEach(function(x,i){
      abc2 += "<div class='col-xs-12' style='margin-bottom:10px'><div class='btn btn-success batches' id='attendance#" + x._id + "' onclick='controller.routes(event)'>" + x.semester + "th, Sem" + "</div></div>";
    })
  ...
  scoreList: function(){
    ...
    model.personalInfo.current_classes.forEach(function(x,i){
      abc1 += "<div class='col-xs-12' style='margin-bottom:10px'><div class='btn btn-warning batches' id='score#" + x._id + "' onclick='controller.routes(event)'>" + x.semester + "th, Sem" + "</div></div>";
    })
    abc1 += "</div></div>";
  ...
  loadReportSection: function(){
   ...
   document.getElementById('mobile-report-section').style.color = "black";
    ...
    model.personalInfo.current_classes.forEach(function(x,i){
      ...
      contentX += "<div class='row text-center' style='margin-bottom:5px;border: solid 1px rgba(200,200,200,.5);border-bottom:solid 2px rgba(200,200,200,.7);box-shadow: 1px 1px 2px rgba(205,205,205,.6);border-radius:5px'><div class='col-xs-12' style='background-color:white;'><div class='row' style=';padding-bottom:10px'><div class='col-xs-3' style='font-size:40px;'><div class='row'><div class='col-xs-12;' style='color:rgba(0,0,0,.55);color:#ffe030'>" + x.semester + "<span style='font-size:24px'>th</span></div></div></div><div class='col-xs-7'><h3 style='margin-top:10px;color:rgba(0,0,0,.8);margin-bottom:0px;'>" + x.subject + "</h3><div class='row' style='margin-top:0px'><div class='col-xs-12' style='font-size:13px;color:rgba(0,0,0,.4)'>strength -</div></div></div></div><div class='row' style='margin-top:10px;border-bottom:solid 1px rgba(0,0,0,.1);padding-bottom:10px;padding-right:5px'>" + classes_timing + "</div><div class='row' style='padding-top:10px;padding-bottom:10px;font-size:18px;margin-top:0px'><div class='col-xs-4'><span class='glyphicon glyphicon-envelope' style='color:#4286f4'></span></div><div class='col-xs-4 lelo' id='report#"+x._id+"'><span id='report#"+x._id+"' class='glyphicon glyphicon-circle-arrow-right'></span></div><div class='col-xs-4'><span class='glyphicon glyphicon glyphicon-option-horizontal'></span></div></div></div></div>";
    })
